...
extern uint8_t darcode[21][21];


/****************************************************************************
* Function Name  : Drow_darcode
* Description    : 在彩屏上面画二维码
* Input          : x：点的X起始坐标
*                * y：点的Y起始坐标     
*                * color：点的颜色
*                * size: 二维码尺寸
...
extern unsigned char m_byModuleData[177][177]; // [x][y]

void Drow_darcode(uint16_t x, uint16_t y, uint8_t size)
{  
    uint8_t i,j,k,l,mult_base,mult_count= 0;
    uint8_t tmpx,tmpy,tmp_mult_y,tmp_mult_x = 0;
    ...
    for(i = 0; i < size; i++)
    {
        for (j = 0; j < size; ++j)
        {
            for (k = 0; k < mult_base; k++)
            {                
                for (l = 0; l < mult_base; l++)
                {
                    TFT_SetWindow(x, y, x, y);  //设置点的位置
                    if(m_byModuleData[i][j])
                    {
                        TFT_WriteData(BLACK);       //画点
                    }
                    y=y+1;
                }
                y=tmp_mult_y;
...
{   			    
    ...
    {   
        ...
        for(t1=0;t1<8;t1++)
		{
			...
			else LCD_Draw_Point(x,y,GBLUE);   //画空白点（使用背景色）
			temp<<=1;
			y++;
			if((y-y0)==16)
			{
...
{   			    
    ...
    {   
		...
		{
			if(temp&0x80)LCD_Draw_Point(x,y,color);//画实心点
			else LCD_Draw_Point(x,y,WHITE);   //画空白点（使用背景色）
			temp<<=1;
			y++;
			if((y-y0)==24)
			{
